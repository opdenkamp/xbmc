...
void CGUIWindowPVRGuide::GetContextButtons(int itemNumber, CContextButtons &buttons) const
{
  ...
  else if (pItem->GetEPGInfoTag()->EndAsLocalTime() > CDateTime::GetCurrentDateTime())
  {
    ...
    buttons.Add(CONTEXT_BUTTON_START_RECORD_SERIE, 19270); /* Record a Series */
  }
...
bool CGUIWindowPVRGuide::OnContextButtonRecordSeries(CFileItem *item, CONTEXT_BUTTON button)
{
  bool bReturn = false;
